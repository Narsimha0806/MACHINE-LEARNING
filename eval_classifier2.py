# -*- coding: utf-8 -*-
"""eval_classifier2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1W56rtI40vJkr7sAFPrZO6lA9PDBwVcWF
"""
...
sepal_length_width_features = Iris_dataset.data[:, :2] #we use the sepallength and sepalwidth as features
class_labels = Iris_dataset.target  #class labels as target

# here we Split the dataset into training and testing sets
train_features, test_features, train_labels, test_labels = train_test_split(sepal_length_width_features, class_labels, test_size=0.1, random_state=42)

log_reg_model = MultiClassLogisticRegressor(lr=0.01, epochs=1000)
log_reg_model.fit(train_features, train_labels)

np.save('trained_weights_sepal.npy', log_reg_model.weights) #save weights
np.save('trained_bias_sepal.npy', log_reg_model.bias) #save bias
...
Iris_dataset = load_iris()
sepal__length_width_features = Iris_dataset.data[:, :2]  # here we Use sepal length and sepalwidth as features
target_classes = Iris_dataset.target  # Class labels as target

# here we Split the dataset into training and testing
train_data, test_data, train_labels, test_labels = train_test_split(sepal_length_width_features, target_classes, test_size=0.1, random_state=42)

loaded_weights = np.load('trained_weights_sepal.npy') #saved weights and bias are loaded here .
loaded_bias = np.load('trained_bias_sepal.npy')
...
plt.title("sepal Length/Width", fontsize=12, fontweight='bold')
